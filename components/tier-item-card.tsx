...
export function TierItemCard({ item, selected, onInspect }: { item: TierItem; selected?: boolean; onInspect?: () => void }) {
  ...
  return <article className={cn("group overflow-hidden rounded-lg border bg-[#1c201b] transition", selected ? "border-[#c8ad76] ring-1 ring-[#c8ad76]/30" : "border-white/10 hover:border-[#c8ad76]/60")}>
    <button type="button" onClick={onInspect} aria-label={`Examiner ${itemTitle(item)}`} className="focus-ring relative block w-full text-left">
      <div className={cn("relative overflow-hidden bg-[#252a23]", character ? "aspect-[4/5]" : "aspect-[4/3]")}>
