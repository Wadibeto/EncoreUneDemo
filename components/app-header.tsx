...
export function AppHeader({ profile }: { profile: Profile }) {
  return (
    <header className="court-nav sticky top-0 z-40 border-b border-border bg-background/95 backdrop-blur-xl">
      <div className="mx-auto flex min-h-[78px] max-w-[1440px] flex-wrap items-center gap-x-5 gap-y-3 px-4 py-3 sm:px-8 lg:px-12">
        ...
        <nav aria-label="Navigation principale" className="order-3 flex w-full gap-6 border-t border-border pt-3 text-xs sm:order-none sm:ml-3 sm:w-auto sm:border-0 sm:pt-0">
          <Link href="/dashboard" className="focus-ring court-link rounded-sm py-1 text-foreground">Vos parties</Link>
          <Link href="/games" className="focus-ring court-link rounded-sm py-1 text-slate-400">Jeux</Link>
        </nav>
