...
import { Check, Download, ImageDown, LoaderCircle, Palette, RotateCcw, Search, Shield, WifiOff } from "lucide-react";
...
export function TierBoard({ listId, initialTitle, initialItems, initialConfig, initialRevision, category, currentUser, members: initialMembers }: { listId: string; initialTitle: string; initialItems: TierItem[]; initialConfig: TierDefinition[]; initialRevision: number; category: TierCategory; currentUser: Profile; members: Profile[] }) {
  ...
  return <>
    <div className="mb-7 flex flex-wrap items-end justify-between gap-5"><div><p className="eyebrow">{category === "characters" ? "Armures & DA" : "Jeux"}</p><h1 className="display-font mt-3 text-4xl leading-tight text-[#f4ead7] sm:text-5xl">{title}</h1></div><div className="flex items-baseline gap-2"><span className="display-font text-4xl text-[#c8ad76]">{rankedCount}</span><span className="text-xs text-stone-500">/ {items.length} classés</span></div></div>
    ...
    <div className={cn("grid items-start gap-5", selected ? "xl:grid-cols-[minmax(0,1fr)_340px]" : "xl:grid-cols-[minmax(0,1fr)_270px]")}>
      <div className="min-w-0">
        <DndContext sensors={sensors} collisionDetection={detectDropTarget} onDragStart={(e) => setActiveId(String(e.active.id))} onDragCancel={() => setActiveId(null)} onDragEnd={onDragEnd} accessibility={{ screenReaderInstructions: { draggable: "Pour déplacer une carte, appuyez sur Espace, utilisez les flèches, puis Espace pour déposer. Échap annule. Vous pouvez aussi choisir un rang dans la fiche." }, announcements: { onDragStart: ({ active }) => `${itemTitle(items.find((item) => item.id === active.id)!)} sélectionné.`, onDragOver: ({ over }) => over ? "Zone de dépôt atteinte." : "Hors d’une zone de dépôt.", onDragEnd: ({ over }) => over ? "Carte déposée. Enregistrement du classement." : "Déplacement annulé.", onDragCancel: () => "Déplacement annulé." } }}>
          <div ref={boardRef} className="court-frame space-y-2 rounded-sm bg-[#181515] p-2 sm:p-3">
          ...
          <section className="mt-6" aria-label="Cartes à classer"><div className="mb-3 flex flex-wrap items-center justify-between gap-3"><div><p className="eyebrow">La réserve</p><p className="mt-1 text-xs text-stone-500">Cliquez pour examiner · poignée pour déplacer</p></div><div className="flex max-w-full flex-wrap gap-2"><label className="relative"><Search className="pointer-events-none absolute left-3 top-2.5 size-3.5 text-stone-500" /><input aria-label="Rechercher dans la réserve" placeholder="Rechercher…" value={query} onChange={(e) => setQuery(e.target.value)} className="focus-ring h-9 w-52 max-w-full rounded-lg border border-white/10 bg-black/15 pl-9 pr-3 text-xs" /></label>{category === "characters" && <select aria-label="Filtrer la réserve par jeu" value={gameFilter} onChange={(e) => setGameFilter(e.target.value)} className="focus-ring h-9 rounded-lg border border-white/10 bg-[#211b18] px-2 text-xs"><option value="all">Les deux univers</option><option value="crown-gambit">Crown Gambit</option><option value="sovereign-tower">Sovereign Tower</option></select>}</div></div><TierRow tierKey="unranked" label={unranked.label} color={unranked.color} items={filtered} selectedId={selectedId} onInspect={setSelectedId} disabled={disabled} unranked /></section>
          <DragOverlay>{active ? <div className="w-[116px] rotate-3 opacity-90"><TierItemCard item={active} /></div> : null}</DragOverlay>
        </DndContext>
      </div>
      {selected ? <CharacterInspector item={selected} config={tierConfig} busy={disabled} onClose={() => setSelectedId(undefined)} onMove={(tier) => { void move(selected.id, tier); }} onVariant={(id) => { void mutate(() => supabase.rpc("set_tier_item_variant", { p_list_id: listId, p_item_id: selected.id, p_variant_id: id })); }} /> : <aside className="court-frame p-6 xl:sticky xl:top-24"><Shield className="mb-5 size-8 text-primary" strokeWidth={1} /><h2 className="display-font text-2xl">Une carte à la loupe</h2><p className="mt-3 text-xs leading-6 text-slate-400">Cliquez sur une carte pour voir les détails.</p><div aria-hidden="true" className="mt-8 flex items-center gap-3 text-primary/50"><span className="h-px flex-1 bg-current" />✦<span className="h-px flex-1 bg-current" /></div></aside>}
    </div>
    {category === "characters" && <details className="mt-6 max-w-3xl text-[11px] leading-relaxed text-stone-500"><summary className="focus-ring w-fit cursor-pointer hover:text-primary">Sources & crédits</summary><p className="mt-2">{CATALOG_COVERAGE_NOTE} Illustrations © leurs ayants droit.</p></details>}
