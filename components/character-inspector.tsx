...
export function CharacterInspector({ item, config, busy, onClose, onMove, onVariant }: { item: TierItem; config: TierDefinition[]; busy: boolean; onClose: () => void; onMove: (tier: string) => void; onVariant: (id: string) => void }) {
  ...
  const art = (large: boolean) => <div className={cn("relative overflow-auto rounded-xl border border-white/10", background === "light" ? "text-[#262c22]" : "text-[#f0e8d7]", large ? "h-[60vh]" : "h-72 sm:h-80")} style={{ background: background === "light" ? "#e4ddc9" : background === "gray" ? "#756c65" : "#191616" }}>
  ...
  return <aside ref={inspectorRef} className="panel scroll-mt-36 rounded-sm p-4 xl:sticky xl:top-24" aria-label="Fiche du personnage">
    ...
    {character && <><p className="mt-5 text-xs font-semibold text-[#dec8a0]">{character.role}</p><p className="mt-2 text-xs leading-relaxed text-stone-400">{character.description}</p>
      ...
      <label className="mt-5 block text-xs text-stone-400">Visuels disponibles · {character.variants.length}<select value={variant?.id} onChange={(event) => { setPreviewId(event.target.value); setReveal(false); }} className="focus-ring mt-2 w-full rounded-lg border border-white/10 bg-[#211b18] p-2.5 text-sm text-[#e9dfcb]">{character.variants.map((entry) => <option key={entry.id} value={entry.id}>{entry.label}{entry.spoiler ? " · spoiler" : ""}</option>)}</select></label>
      {variant?.id !== chosen?.id && <Button className="mt-2 w-full" size="sm" disabled={busy || locked} onClick={() => variant && onVariant(variant.id)}>Choisir ce visuel</Button>}
      <p className="mt-2 text-[10px] leading-relaxed text-stone-500">{character.variants.length === 1 ? "Un seul visuel documenté pour le moment." : "Visuel partagé avec votre duo."}</p>
    ...
    <div className="mt-5 border-t border-white/10 pt-4"><label className="block text-xs text-stone-400">Placer dans un rang<select value={item.tier} disabled={busy} onChange={(event) => onMove(event.target.value)} className="focus-ring mt-2 w-full rounded-lg border border-white/10 bg-[#211b18] p-2.5 text-sm text-[#e9dfcb]">{config.map((tier) => <option key={tier.key} value={tier.key}>{tier.label}</option>)}</select></label></div>
