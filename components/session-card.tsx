...
export function SessionCard({ kind, id, title, updatedAt, status, category = "games" }: {
  ...
  return (
    <Link href={guess ? `/guess/${id}` : `/tierlists/${id}`} className="focus-ring court-card group relative flex min-h-48 flex-col rounded-sm border border-border bg-card p-5 transition duration-300 hover:-translate-y-1 hover:border-primary/60 hover:bg-[#2b211c]">
      ...
      <div className="mt-auto flex flex-wrap items-center justify-between gap-2 pt-5 text-[11px] text-slate-400">
        ...
        {!guess && <span className="text-primary">Reprendre →</span>}
      </div>
    </Link>
  );
}
